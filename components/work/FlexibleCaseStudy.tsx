      ...
      <div className="border-t border-mist [&>section]:border-t [&>section]:border-black/5">
        {sections.map((block, i) => {
          ...
          switch (block.blockType) {
              ...
              return (
                <section key={key} className="bg-white">
                  <div className="mx-auto max-w-edge px-5 py-16 sm:px-8 sm:py-24">
                    {block.eyebrow && <p className="mb-4 text-xs font-bold uppercase tracking-[0.2em] text-geek-cyan">{block.eyebrow}</p>}
                    <h2 className="h-display max-w-[20ch] text-[clamp(2rem,5vw,4rem)] uppercase text-ink">{block.heading}</h2>
                    {block.body && <p className="mt-5 max-w-[60ch] text-lg leading-relaxed text-graphite">{block.body}</p>}
                  </div>
                </section>
              );
            case "richText":
              return (
                <section key={key} className="bg-white">
                  <div className="prose-geek mx-auto max-w-[70ch] px-5 py-14 text-lg leading-relaxed text-ink/80 sm:px-8">
                ...
                <section key={key} className="bg-white">
                  <div className="mx-auto max-w-edge px-5 py-12 sm:px-8">
                    ...
                    {(block.caption || block.credit) && (
                      <p className="mt-3 text-[11px] text-graphite">{[block.caption, block.credit].filter(Boolean).join(" · ")}</p>
                    )}
                  </div>
                </section>
              );
              ...
              return (
                <section key={key} className="bg-white">
                  <div className="mx-auto grid max-w-edge grid-cols-1 items-center gap-8 px-5 py-16 sm:px-8 md:grid-cols-2">
                    ...
                    <div className="prose-geek text-lg leading-relaxed text-ink/80"><RichText data={block.content} /></div>
                  </div>
                </section>
              );
            }
              ...
              return (
                <section key={key} className="bg-white">
                  <div className="mx-auto max-w-edge px-5 py-16 sm:px-8">
                    {block.heading && <p className="mb-6 text-xs font-bold uppercase tracking-[0.2em] text-geek-cyan">{block.heading}</p>}
            ...
            case "metrics":
              return (
                <section key={key} className="bg-paper">
                  <div className="mx-auto max-w-edge px-5 py-16 sm:px-8 sm:py-20">
                    {block.heading && <p className="mb-8 text-xs font-bold uppercase tracking-[0.2em] text-geek-cyan">{block.heading}</p>}
                    <div className="grid grid-cols-1 gap-x-8 gap-y-10 sm:grid-cols-2 lg:grid-cols-3">
                      {(block.items ?? []).map((s, j) => (
                        <div key={j}>
                          ...
                          <p className="mt-3 text-sm font-semibold uppercase tracking-[0.14em] text-graphite">{s.label}</p>
                          {s.note && <p className="mt-1 text-[11px] text-graphite/70">{s.note}</p>}
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            case "cta":
              return (
                <section key={key} className="bg-white">
                  <div className="mx-auto max-w-edge px-5 py-20 text-center sm:px-8">
                    <h2 className="h-display mx-auto max-w-[20ch] text-[clamp(1.8rem,5vw,3.6rem)] uppercase text-ink">{block.heading}</h2>
                    {block.body && <p className="mx-auto mt-5 max-w-[52ch] text-lg text-graphite">{block.body}</p>}
